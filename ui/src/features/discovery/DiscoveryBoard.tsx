...
import { useAliases, useDiscovery, useProviders, useStale, useRefreshProvider } from "@/lib/queries";
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button variant="outline" onClick={refreshAll} disabled={refresh.isPending || providers.data.length === 0}>
              ...
              <div key={p.id} className="flex flex-col rounded-lg border border-border bg-card">
                ...
                <div className="flex items-center gap-2 px-4 py-2 text-xs text-muted-foreground">
                  <Badge variant={snap?.error ? "destructive" : snap ? "success" : "secondary"}>
                    {snap?.error ? "error" : snap ? `${snap.count} models` : "no data"}
                  </Badge>
                  {snap?.fetched_at && <span>fetched {formatRelative(snap.fetched_at)}</span>}
                </div>
