...
- Docker + Compose v2
- Puerto libre (default host `8080`, o remap)
...
```bash
docker build -t aigw .
docker run -d --name aigw -p 8080:8080 -v aigw-data:/app/data \
...
1. Abrir `https://<host>/admin/` (o `http://localhost:8080/admin/` en local).
...
Terminar TLS en el proxy. El contenedor escucha HTTP en `:8080`.
...
Docker (`http://aigw:8080`) en lugar de `127.0.0.1:8080`.
...
| `AIGW_ADDR` | `:8080` (CMD imagen) | raramente hace falta |
